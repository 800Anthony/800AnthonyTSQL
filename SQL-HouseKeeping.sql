...
SET @BackupDirectory     = N'C:\SQLBackups'
...
BEGIN
  ...
  SET @JobName01 = 'DBA - Backup Database Full - All Databases'
  ...
  SET @JobName04 = 'DBA - Backup Transaction Log - All Databases'
  SET @JobCommand04 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''ALL_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''LOG'', @Verify = ''Y'', @CleanupTime = ' + ISNULL(CAST(@CleanupTime AS nvarchar),'NULL') + ', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobName05 = 'DBA - Integrity Check - All Databases'
  ...
  SET @JobName07 = 'DBA - Index Optimize - All Databases'
  SET @JobCommand07 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[IndexOptimize] @Databases = ''ALL_DATABASES''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobName08 = 'DBA - Delete Backup History'
  ...
  SET @JobName09 = 'DBA - Purge Job History'
  ...
  EXECUTE msdb.dbo.sp_add_jobschedule @job_name=@JobName01, @name='Full Database Backup Schedule',     @freq_type=4, @freq_interval=1,                                                @active_start_time=210000
  EXECUTE msdb.dbo.sp_add_jobschedule @job_name=@JobName04, @name='Transaction Log Backup Schedule',   @freq_type=4, @freq_interval=1,  @freq_subday_type=8, @freq_subday_interval=1, @active_start_time=000000
  EXECUTE msdb.dbo.sp_add_jobschedule @job_name=@JobName05, @name='Database Integrity Check Schedule', @freq_type=8, @freq_interval=1,  @freq_recurrence_factor=1,                    @active_start_time=020000
  EXECUTE msdb.dbo.sp_add_jobschedule @job_name=@JobName07, @name='Database Index Optimise Schedule',  @freq_type=8, @freq_interval=64, @freq_recurrence_factor=1,                    @active_start_time=020000
  EXECUTE msdb.dbo.sp_add_jobschedule @job_name=@JobName08, @name='Cleanup Schedule',                  @freq_type=8, @freq_interval=1,  @freq_recurrence_factor=1,                    @active_start_time=100000
  EXECUTE msdb.dbo.sp_attach_schedule @job_name=@JobName09, @schedule_name='Cleanup Schedule'
  EXECUTE msdb.dbo.sp_attach_schedule @job_name=@JobName10, @schedule_name='Cleanup Schedule'
  EXECUTE msdb.dbo.sp_attach_schedule @job_name=@JobName11, @schedule_name='Cleanup Schedule'
  --
  -- Add Cycle Errorlog job with weekly schedule.
  --
  IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = 'DBA - Cycle SQL Errorlog')
  BEGIN
    EXECUTE msdb.dbo.sp_add_job         @job_name = 'DBA - Cycle SQL Errorlog', @description = 'Cycle SQL Server Errorlog', @category_name = @JobCategory, @owner_login_name = @JobOwner
    EXECUTE msdb.dbo.sp_add_jobstep     @job_name = 'DBA - Cycle SQL Errorlog', @step_name = 'Cycle SQL Errorlog', @subsystem = 'TSQL', @command = 'EXEC sp_cycle_errorlog', @flags=4
    EXECUTE msdb.dbo.sp_add_jobserver   @job_name = 'DBA - Cycle SQL Errorlog'
    EXECUTE msdb.dbo.sp_add_jobschedule @job_name=  'DBA - Cycle SQL Errorlog', @name='Cycle Errorlog Schedule', @freq_type=8, @freq_interval=2, @freq_recurrence_factor=1, @active_start_time=020000
  END
